...
if [ "$field" = "completion" ]; then
  ...
  expression='s/^[^x].*$//; s/x\s*(\([A-Z]\))?\s*([0-9]{4}-[0-9]{2}-[0-9]{2}).*/\2/; s/^[^0-9].*$//'
elif [ "$field" = "creation-date" ]; then
  expression='s/x\s*(\([A-Z]\))?\s*([0-9]{4}-[0-9]{2}-[0-9]{2})\s*([0-9]{4}-[0-9]{2}-[0-9]{2}).*$/\3/; s/o?\s*(\([A-Z]\))?\s*([0-9]{4}-[0-9]{2}-[0-9]{2}).*$/\2/; s/^[^0-9].*$//'
